---
title: "循环神经网络 (RNN / LSTM / GRU) 详解"
description: "序列建模的经典架构，从基础 RNN 到现代门控机制的完整解析。"
date: 2025-08-17
tags: [深度学习, 科研笔记, 模型架构, 深度学习, RNN, LSTM]
layout: single
author_profile: true
share: true
toc: true
...
给定输入序列 {::nomarkdown}$x_1, x_2, \ldots, x_T${:/nomarkdown}，RNN 在每个时间步 {::nomarkdown}$t${:/nomarkdown} 更新隐藏状态：
...
其中：
- {::nomarkdown}$h_t \in \mathbb{R}^d${:/nomarkdown}：时刻 {::nomarkdown}$t${:/nomarkdown} 的隐藏状态
- {::nomarkdown}$W_{hh} \in \mathbb{R}^{d \times d}${:/nomarkdown}：隐藏状态到隐藏状态的权重
- {::nomarkdown}$W_{xh} \in \mathbb{R}^{d \times d_{in}}${:/nomarkdown}：输入到隐藏状态的权重
- {::nomarkdown}$W_{hy} \in \mathbb{R}^{d_{out} \times d}${:/nomarkdown}：隐藏状态到输出的权重
...
展开 {::nomarkdown}$k${:/nomarkdown} 步后的梯度：

$$\frac{\partial \mathcal{L}}{\partial h_t} = \frac{\partial \mathcal{L}}{\partial h_T} \prod_{i=t}^{T-1} \frac{\partial h_{i+1}}{\partial h_i}$$

其中 {::nomarkdown}$\frac{\partial h_{i+1}}{\partial h_i} = W_{hh}^T \cdot \text{diag}(\tanh'(...))${:/nomarkdown}。

- 当 {::nomarkdown}$W_{hh}${:/nomarkdown} 特征值 {::nomarkdown}$< 1${:/nomarkdown}：梯度指数衰减 → **梯度消失**
- 当 {::nomarkdown}$W_{hh}${:/nomarkdown} 特征值 {::nomarkdown}$> 1${:/nomarkdown}：梯度指数增长 → **梯度爆炸**
...
**遗忘门（Forget Gate）** {::nomarkdown}$f_t = \sigma(W_f \cdot [h_{t-1}, x_t] + b_f)${:/nomarkdown}
- 决定保留多少旧细胞状态信息
- {::nomarkdown}$f_t \to 0${:/nomarkdown}：遗忘；{::nomarkdown}$f_t \to 1${:/nomarkdown}：保留

**输入门（Input Gate）** {::nomarkdown}$i_t = \sigma(W_i \cdot [h_{t-1}, x_t] + b_i)${:/nomarkdown}
- 决定将多少新信息写入细胞状态
- 与候选值 {::nomarkdown}$\tilde{C}_t${:/nomarkdown} 配合使用

**候选细胞状态（Candidate Cell State）** {::nomarkdown}$\tilde{C}_t = \tanh(W_C \cdot [h_{t-1}, x_t] + b_C)${:/nomarkdown}
- 生成新的候选信息

**输出门（Output Gate）** {::nomarkdown}$o_t = \sigma(W_o \cdot [h_{t-1}, x_t] + b_o)${:/nomarkdown}
- 决定输出多少细胞状态信息到隐藏状态
...
当 {::nomarkdown}$f_t = 1${:/nomarkdown} 时，{::nomarkdown}$C_t = C_{t-1} + \Delta${:/nomarkdown}，梯度可以通过细胞状态**无衰减地反向传播**。这被称为 Constant Error Carousel (CEC)。

### 2.5 参数量

LSTM 有 4 组参数（遗忘门、输入门、候选值、输出门），每组权重矩阵大小为 {::nomarkdown}$d \times (d + d_{in})${:/nomarkdown}：
...
- **重置门** {::nomarkdown}$r_t${:/nomarkdown}：控制忽略多少历史信息
  - {::nomarkdown}$r_t \to 0${:/nomarkdown}：忽略过去，只关注当前输入（适合检测新主题）
  - {::nomarkdown}$r_t \to 1${:/nomarkdown}：充分利用历史信息
- **更新门** {::nomarkdown}$z_t${:/nomarkdown}：控制保留多少历史 vs 新信息
  - {::nomarkdown}$z_t \to 0${:/nomarkdown}：保留全部历史（类似 LSTM 的 {::nomarkdown}$f_t=1${:/nomarkdown}）
  - {::nomarkdown}$z_t \to 1${:/nomarkdown}：完全使用新信息
...
| 状态 | 细胞状态 + 隐藏状态 | 仅隐藏状态 |
| 参数量 | {::nomarkdown}$4d(d + d_{in})${:/nomarkdown} | {::nomarkdown}$3d(d + d_{in})${:/nomarkdown} |
...
- **Dropout**：应用于层间连接（非循环连接），{::nomarkdown}$\hat{h}_t^{(l)} = \text{Dropout}(h_t^{(l)})${:/nomarkdown}
...
Encoder-Decoder 结构：
- **Encoder**：将输入序列编码为上下文向量 {::nomarkdown}$c${:/nomarkdown}
- **Decoder**：从 {::nomarkdown}$c${:/nomarkdown} 自回归生成输出序列
...
所有输入信息被压缩到单个固定维度的向量 {::nomarkdown}$c${:/nomarkdown} 中，长序列信息丢失严重。
...
| 方式 | 公式 |
|------|------|
| Dot | {::nomarkdown}$e_{t,i} = s_t^T h_i${:/nomarkdown} |
| General | {::nomarkdown}$e_{t,i} = s_t^T W_a h_i${:/nomarkdown} |
| Concat | {::nomarkdown}$e_{t,i} = v_a^T \tanh(W_a[s_t; h_i])${:/nomarkdown} |

### 7.3 关键区别

- Bahdanau：使用 {::nomarkdown}$s_{t-1}${:/nomarkdown}（上一时刻隐藏状态），先计算注意力，再生成当前输出
- Luong：使用 {::nomarkdown}$s_t${:/nomarkdown}（当前时刻隐藏状态），先生成当前输出，再计算注意力
...
- 任意两个位置间的路径长度恒为 {::nomarkdown}$O(1)${:/nomarkdown}
- 训练时完全并行化
- 更适合 GPU 计算模式
...
- 极长序列（Transformer 的 {::nomarkdown}$O(n^2)${:/nomarkdown} 复杂度是瓶颈）
