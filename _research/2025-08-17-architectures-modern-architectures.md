---
title: "现代架构演进：MoE、SSM 与注意力变体"
description: "Transformer 一统天下之后的演进：稀疏专家 (MoE) 提升参数效率、状态空间模型 (SSM/Mamba) 提供线性复杂度循环、Grouped-Query / Multi-head Latent Attention 等变体降低 KV cache 成本，以及长上下文位置编码的外推方案。"
date: 2025-08-17
tags: [深度学习, 科研笔记, 模型架构, 深度学习]
layout: single
author_profile: true
share: true
toc: true
...
- **长上下文的二次成本**：标准 Self-Attention 对长度 {::nomarkdown}$L${:/nomarkdown} 的序列需要 {::nomarkdown}$O(L^2)${:/nomarkdown} 的计算与显存，无论是 4k、32k 还是百万 token，平方增长都极不友好。
...
密集 FFN 在每个 token 上都激活全部参数。MoE 的核心想法是：**把一个大的 FFN 拆成 {::nomarkdown}$N${:/nomarkdown} 个独立的"专家"FFN，每个 token 只激活其中 top-k 个**。这样模型总参数量很大（容量高），但单 token 的实际计算量只相当于一个更小的密集模型（FLOPs 低）。
...
MoE 层用一个门控网络 {::nomarkdown}$G(x)${:/nomarkdown} 决定 token 应送往哪些专家。最常见的形式是 top-k 路由：

$$G(x) = \text{softmax}\big(\text{top-k}(W_g\, x)\big)$$

即先用线性矩阵 {::nomarkdown}$W_g \in \mathbb{R}^{N \times d}${:/nomarkdown} 计算每个专家的 logit，取最大的 {::nomarkdown}$k${:/nomarkdown} 个，**只对这 {::nomarkdown}$k${:/nomarkdown} 个做 softmax，其余专家的门控权重置 0**。最终该 token 的 MoE 输出为

$$y = \sum_{i \in \text{top-k}} G_i(x)\, E_i(x)$$

其中 {::nomarkdown}$E_i(\cdot)${:/nomarkdown} 是第 {::nomarkdown}$i${:/nomarkdown} 个专家 FFN。未被选中的专家不参与计算，从而实现稀疏激活。

### 2.3 负载均衡辅助损失

MoE 训练有一个臭名昭著的失败模式：**路由塌缩 (router collapse)**——门控网络学会把几乎所有 token 都送到少数几个"好用"的专家，其余专家得不到训练。这会让有效专家数远小于名义专家数，参数利用率急剧下降。

Switch Transformer 引入了一个辅助损失来鼓励专家被均匀使用。设 batch 内共 {::nomarkdown}$T${:/nomarkdown} 个 token、{::nomarkdown}$N${:/nomarkdown} 个专家，记 {::nomarkdown}$f_i${:/nomarkdown} 为送到专家 {::nomarkdown}$i${:/nomarkdown} 的 token 比例，{::nomarkdown}$P_i${:/nomarkdown} 为门控给专家 {::nomarkdown}$i${:/nomarkdown} 的平均概率：

$$f_i = \frac{1}{T}\sum_{x} \mathbb{1}\{i \in \text{top-k}(x)\}, \quad P_i = \frac{1}{T}\sum_{x} G_i(x)$$

辅助损失定义为两者乘积的 {::nomarkdown}$N${:/nomarkdown} 倍：

$$\mathcal{L}_{\text{aux}} = \alpha \cdot N \cdot \sum_{i=1}^{N} f_i\, P_i$$

当所有专家被均匀使用时 {::nomarkdown}$f_i = 1/N${:/nomarkdown}、{::nomarkdown}$P_i = 1/N${:/nomarkdown}，loss 取极小值 {::nomarkdown}$\alpha${:/nomarkdown}。{::nomarkdown}$f_i${:/nomarkdown} 不可导，{::nomarkdown}$P_i${:/nomarkdown} 可导，二者乘积把梯度回传到门控网络，从而把流量推向欠载专家。
...
状态空间模型源自控制论中的线性时不变 (LTI) 系统：输入 {::nomarkdown}$x(t)${:/nomarkdown} 经隐状态 {::nomarkdown}$h(t)${:/nomarkdown} 映射到输出 {::nomarkdown}$y(t)${:/nomarkdown}：

$$h'(t) = A\, h(t) + B\, x(t), \quad y(t) = C\, h(t)$$

其中 {::nomarkdown}$A \in \mathbb{R}^{N \times N}${:/nomarkdown}、{::nomarkdown}$B \in \mathbb{R}^{N \times 1}${:/nomarkdown}、{::nomarkdown}$C \in \mathbb{R}^{1 \times N}${:/nomarkdown}（单输入单输出形式；多维输入输出推广同理）。通过零阶保持 (Zero-Order Hold, ZOH) 离散化，给定步长 {::nomarkdown}$\Delta${:/nomarkdown}，得到离散参数 {::nomarkdown}$\bar{A}, \bar{B}, \bar{C}${:/nomarkdown} 后即可在序列上 rollout：

$$h_t = \bar{A}\, h_{t-1} + \bar{B}\, x_t, \quad y_t = \bar{C}\, h_t$$

这是一个线性循环，因此既可像 RNN 那样逐步递推（推理 {::nomarkdown}$O(L)${:/nomarkdown}），也可借助卷积/扫描并行训练。

### 3.2 S4：结构化状态空间

直接学一个 {::nomarkdown}$N \times N${:/nomarkdown} 矩阵 {::nomarkdown}$A${:/nomarkdown} 既难收敛又不利长程建模。**S4 (Gu et al., 2021)** 的关键贡献是用 HiPPO 矩阵初始化 {::nomarkdown}$A${:/nomarkdown}，使其隐状态对近端历史有指数衰减的记忆、对远端历史有压缩表示，从而获得远超普通 RNN 的长序列建模能力。S4 还通过结构化参数化（对角加低秩）把推理时的卷积运算降到 {::nomarkdown}$O(L)${:/nomarkdown}。

### 3.3 Mamba 与选择性 SSM

S4 仍是 LTI 系统：{::nomarkdown}$A, B, C, \Delta${:/nomarkdown} 与输入无关，对所有 token 一视同仁。这让它在长序列"信息检索"上力不从心——它无法根据内容选择性地记住或遗忘。

**Mamba (Gu & Dao, 2023)** 的核心突破是 **选择性 SSM (Selective SSM)**：让 {::nomarkdown}$B${:/nomarkdown}、{::nomarkdown}$C${:/nomarkdown} 与离散化步长 {::nomarkdown}$\Delta${:/nomarkdown} 依赖于输入 {::nomarkdown}$x_t${:/nomarkdown}：

$$B_t = B(x_t), \quad C_t = C(x_t), \quad \Delta_t = \Delta(x_t)$$

{::nomarkdown}$\Delta_t${:/nomarkdown} 控制离散化"步长"，可理解为一种门控：{::nomarkdown}$\Delta_t${:/nomarkdown} 大时新输入强烈更新隐状态（"记住"），{::nomarkdown}$\Delta_t${:/nomarkdown} 小时几乎忽略输入（"遗忘"）。这样把 LTI 打破成内容相关、时变的循环，类似门控 RNN，但保持线性复杂度。

### 3.4 硬件感知扫描

选择性参数意味着不能再像 S4 那样用固定卷积核，必须做时变循环。直接实现 {::nomarkdown}$O(L)${:/nomarkdown} 串行扫描在 GPU 上极慢。Mamba 采用 **hardware-aware parallel scan**：把递推写成结合律的扫描算子，用前缀和式的并行算法在 GPU 上高效执行，避免显式物化 {::nomarkdown}$N \times N${:/nomarkdown} 的状态转移矩阵、保持显存与算力高效。
...
|---|---|---|
| 序列复杂度 | {::nomarkdown}$O(L^2)${:/nomarkdown} | {::nomarkdown}$O(L)${:/nomarkdown} |
| 训练并行 | 高（矩阵乘） | 中（parallel scan） |
| 长上下文 | 受 KV cache 显存限制 | 隐状态固定，天然长 |
| 精确检索/复制 | 强 | 弱（LTI 残余 + 线性瓶颈） |
| 推理状态 | KV cache 随 {::nomarkdown}$L${:/nomarkdown} 线性增长 | 固定大小隐状态 |
| 推理延迟 | 带宽受限 | 算力受限 |
...
**GQA (Ainslie et al., 2023)** 取折中：K/V head 数 {::nomarkdown}$G${:/nomarkdown} 介于 1 (MQA) 与 {::nomarkdown}$H${:/nomarkdown} (MHA，标准多头) 之间，每个 K/V head 被一组 query head 共享。

设 query head 数 {::nomarkdown}$H${:/nomarkdown}、K/V head 数 {::nomarkdown}$G${:/nomarkdown}、head 维度 {::nomarkdown}$d${:/nomarkdown}、序列长度 {::nomarkdown}$L${:/nomarkdown}，则推理时 KV cache 大小对比：

| 注意力形式 | KV cache 大小 |
|---|---|
| 密集 MHA | {::nomarkdown}$\sim H \cdot L \cdot d${:/nomarkdown} |
| GQA | {::nomarkdown}$\sim G \cdot L \cdot d${:/nomarkdown} |
| MQA | {::nomarkdown}$\sim L \cdot d${:/nomarkdown} |

GQA 在 {::nomarkdown}$G=1${:/nomarkdown} 退化为 MQA、在 {::nomarkdown}$G=H${:/nomarkdown} 退化为 MHA，因而可在质量与显存间任意插值。**Llama 2 (部分大尺寸) / Llama 3 全系采用 GQA**，已成为现代 LLM 的事实标准配置之一。
...
设隐状态维度 {::nomarkdown}$d${:/nomarkdown}、latent 维度 {::nomarkdown}$d_c \ll d \cdot H${:/nomarkdown}，则每 token：

$$c = W_{\text{down}}\, x \in \mathbb{R}^{d_c}, \quad K, V = \text{reshape}(W_{\text{up}}\, c)$$

推理 KV cache 从 {::nomarkdown}$H \cdot L \cdot d${:/nomarkdown} 降到 {::nomarkdown}$d_c \cdot L${:/nomarkdown}，量级接近 MQA 却几乎不掉点。一个细节难点是 RoPE：latent 上投影后再做旋转位置编码会破坏低秩压缩，故 MLA 采用 **解耦 RoPE**——把位置相关的部分单独做一小段不可压缩的 K，与 latent 恢复出的内容 K 拼接使用。MLA 是 DeepSeek-V2/V3 实现超长上下文 + 低成本推理的关键一环。

### 4.4 线性注意力近似

另一条思路是 **用核技巧把 softmax 注意力近似为线性**，从而把复杂度从 {::nomarkdown}$O(L^2)${:/nomarkdown} 降到 {::nomarkdown}$O(L)${:/nomarkdown}。一般形式（Katharopoulos et al., 2020）：

$$\text{Att}(q, k, v) = \frac{\phi(q)\,\big(\phi(k)^\top v\big)}{\phi(q)\,\sum \phi(k)}$$

由于 {::nomarkdown}$\phi(k)^\top v${:/nomarkdown} 可先算再与 {::nomarkdown}$\phi(q)${:/nomarkdown} 相乘，可分解为类似 RNN 的递推：

$$S_t = S_{t-1} + \phi(k_t)\, v_t^\top, \quad y_t = \frac{\phi(q_t)\, S_t}{\phi(q_t)\, \sum_{i\le t}\phi(k_i)}$$

从而推理时 {::nomarkdown}$O(L)${:/nomarkdown}、训练时仍可并行。**Performer (Choromanski et al., 2020)** 用随机特征 (random features) 近似 softmax，给出无偏估计。
...
现代 LLM 普遍采用 **旋转位置编码 (Rotary Position Embedding, RoPE, Su et al., 2021)**：把 query、key 视作二维平面的复数，按位置 {::nomarkdown}$m${:/nomarkdown} 旋转角度 {::nomarkdown}$m\theta${:/nomarkdown}：

$$\text{RoPE}(x, m) = x\, e^{i m \theta}$$

对一对位置 {::nomarkdown}$m, n${:/nomarkdown} 的 query/key，相对相位为 {::nomarkdown}$(m-n)\theta${:/nomarkdown}，因此 RoPE 天然编码 **相对位置**，且对训练时未见过的更长距离外推能力有限。

### 5.2 外推问题

模型在 {::nomarkdown}$L_{\text{train}}${:/nomarkdown}（如 4k）上训练，推理用到 {::nomarkdown}$L_{\text{test}} > L_{\text{train}}${:/nomarkdown}（如 32k、128k）时，超出训练范围的相对相位会让注意力分布失常、质量骤降。围绕"如何把 RoPE 外推到更长上下文"涌现一批方法。

### 5.3 位置插值 (Position Interpolation)

**Position Interpolation, Chen et al., 2023**：与其外推到未见角度，不如 **把目标位置压缩到训练范围内**——把旋转角度缩放 {::nomarkdown}$s = L_{\text{test}} / L_{\text{train}}${:/nomarkdown}：
...
NTS 系列方法调整 RoPE 基频 {::nomarkdown}$\theta_{\text{base}}${:/nomarkdown}，使低频部分更平滑、高频部分基本不变。直觉上：高频对应细粒度局部位置、应保持原行为；低频对应长程、应平滑外推。
...
- **Sliding Window Attention (Mistral)**：每层只看最近 {::nomarkdown}$W${:/nomarkdown} 个 token，配合层数堆叠让有效感受野随层增长，显存与计算恒定；遇到需要远距离精检索的任务时再叠加少量全局 attention。
