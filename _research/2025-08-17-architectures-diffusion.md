---
title: "扩散模型（Diffusion Models）详解"
description: "从 DDPM 的数学推导到 DDIM、Classifier-Free Guidance 与 Flow Matching 的完整解析。"
date: 2025-08-17
tags: [深度学习, 科研笔记, 模型架构, 深度学习, Diffusion]
layout: single
author_profile: true
share: true
toc: true
...
1. **前向过程（加噪）**：逐步向数据 {::nomarkdown}$\mathbf{x}_0${:/nomarkdown} 添加高斯噪声，直到变成纯噪声 {::nomarkdown}$\mathbf{x}_T \sim \mathcal{N}(0, \mathbf{I})${:/nomarkdown}。
...
给定方差调度 {::nomarkdown}$\{\beta_t\}_{t=1}^T${:/nomarkdown}（通常 {::nomarkdown}$0 < \beta_t < 1${:/nomarkdown}），前向过程定义为马尔可夫链：

$$q(\mathbf{x}_t \mid \mathbf{x}_{t-1}) = \mathcal{N}\left(\mathbf{x}_t; \sqrt{1 - \beta_t}\,\mathbf{x}_{t-1}, \beta_t \mathbf{I}\right)$$

即 {::nomarkdown}$\mathbf{x}_t = \sqrt{1 - \beta_t}\,\mathbf{x}_{t-1} + \sqrt{\beta_t}\,\boldsymbol{\epsilon}_t${:/nomarkdown}，{::nomarkdown}$\boldsymbol{\epsilon}_t \sim \mathcal{N}(\mathbf{0}, \mathbf{I})${:/nomarkdown}。

### 2.2 任意时刻的解析形式（重参数化）

定义 {::nomarkdown}$\alpha_t = 1 - \beta_t${:/nomarkdown}，{::nomarkdown}$\bar{\alpha}_t = \prod_{s=1}^{t} \alpha_s${:/nomarkdown}。利用高斯分布的可加性，可**一步**从 {::nomarkdown}$\mathbf{x}_0${:/nomarkdown} 得到 {::nomarkdown}$\mathbf{x}_t${:/nomarkdown}：
...
**推导**：由 {::nomarkdown}$\mathbf{x}_t = \sqrt{\alpha_t}\mathbf{x}_{t-1} + \sqrt{\beta_t}\boldsymbol{\epsilon}_t${:/nomarkdown} 逐层展开并合并独立高斯噪声（两个独立高斯的加权和仍为高斯，方差相加）：

$$\mathbf{x}_t = \sqrt{\prod_{s=1}^{t}\alpha_s}\,\mathbf{x}_0 + \underbrace{\sqrt{1 - \bar{\alpha}_t}}_{\text{累积噪声方差}}\,\boldsymbol{\epsilon}$$

当 {::nomarkdown}$T${:/nomarkdown} 足够大且 {::nomarkdown}$\bar{\alpha}_T \to 0${:/nomarkdown} 时，{::nomarkdown}$\mathbf{x}_T \approx \mathcal{N}(\mathbf{0}, \mathbf{I})${:/nomarkdown}。
...
反向过程 {::nomarkdown}$p_\theta${:/nomarkdown} 每一步也是高斯（对足够小的 {::nomarkdown}$\beta_t${:/nomarkdown} 的近似），由神经网络参数化：
...
目标：最大化对数似然 {::nomarkdown}$\log p_\theta(\mathbf{x}_0)${:/nomarkdown}。

### 3.2 ELBO 推导

引入前向过程 {::nomarkdown}$q${:/nomarkdown}，由 Jensen 不等式（或重写为期望形式）：
...
- {::nomarkdown}$\mathcal{L}_T${:/nomarkdown}：先验项，无参数（{::nomarkdown}$\mathbf{x}_T${:/nomarkdown} 趋近标准高斯），可视为常数；
- {::nomarkdown}$\mathcal{L}_0${:/nomarkdown}：重建项（最后一步），离散像素时用独立离散解码器；
- {::nomarkdown}$\mathcal{L}_{t-1}${:/nomarkdown}：**去噪匹配项**，是训练的核心。

### 3.3 可解析的高斯后验

关键：当以 {::nomarkdown}$\mathbf{x}_0${:/nomarkdown} 为条件时，{::nomarkdown}$q(\mathbf{x}_{t-1} \mid \mathbf{x}_t, \mathbf{x}_0)${:/nomarkdown} 的均值和方差可解析。由贝叶斯公式对高斯分布求后验：
...
其中（令 {::nomarkdown}$\tilde\beta_t = \beta_t${:/nomarkdown}，{::nomarkdown}$\bar\alpha_t${:/nomarkdown} 如上）：
...
两个高斯 {::nomarkdown}$\mathcal{N}(\boldsymbol{\mu}_1, \sigma_1^2 \mathbf{I})${:/nomarkdown} 与 {::nomarkdown}$\mathcal{N}(\boldsymbol{\mu}_2, \sigma_2^2 \mathbf{I})${:/nomarkdown} 的 KL 散度为：
...
因此 {::nomarkdown}$\mathcal{L}_{t-1}${:/nomarkdown} 等价于使网络均值 {::nomarkdown}$\boldsymbol{\mu}_\theta${:/nomarkdown} 逼近解析后验均值 {::nomarkdown}$\tilde{\boldsymbol{\mu}}_t${:/nomarkdown}：
...
将 {::nomarkdown}$\mathbf{x}_t = \sqrt{\bar\alpha_t}\mathbf{x}_0 + \sqrt{1-\bar\alpha_t}\boldsymbol{\epsilon}${:/nomarkdown} 代入 {::nomarkdown}$\tilde{\boldsymbol{\mu}}_t${:/nomarkdown}，可消去 {::nomarkdown}$\mathbf{x}_0${:/nomarkdown}，得到 {::nomarkdown}$\tilde{\boldsymbol{\mu}}_t${:/nomarkdown} 关于 {::nomarkdown}$\mathbf{x}_t${:/nomarkdown} 与 {::nomarkdown}$\boldsymbol{\epsilon}${:/nomarkdown} 的表达式。令网络改为**预测噪声**：{::nomarkdown}$\boldsymbol{\mu}_\theta${:/nomarkdown} 由 {::nomarkdown}$\boldsymbol{\epsilon}_\theta${:/nomarkdown} 表示，则均值匹配损失等价于（忽略常数系数）：
...
这是实际训练中学到的唯一损失：**随机采样时刻 {::nomarkdown}$t${:/nomarkdown} 与噪声 {::nomarkdown}$\boldsymbol{\epsilon}${:/nomarkdown}，让网络预测注入的噪声**。

### 推理（生成）

采样时从 {::nomarkdown}$\mathbf{x}_T \sim \mathcal{N}(\mathbf{0}, \mathbf{I})${:/nomarkdown} 出发，逐步：
...
- **UNet**：多分辨率编码器-解码器 + 跳跃连接，时间 {::nomarkdown}$t${:/nomarkdown} 通过正弦位置编码 + FiLM/AdaGN 注入。
...
| 调度 | 形式 | 特点 |
|------|------|------|
| Linear（DDPM） | {::nomarkdown}$\beta_t${:/nomarkdown} 从 {::nomarkdown}$10^{-4}${:/nomarkdown} 线性到 {::nomarkdown}$0.02${:/nomarkdown} | 原始方案 |
| Cosine（2021） | 平滑的余弦形式 | 高分辨率训练更稳 |
| Learned | 网络学习 {::nomarkdown}$\beta_t${:/nomarkdown} | 灵活但更复杂 |

---

## 6. DDIM：确定性采样

DDIM（Song et al., 2021）将采样过程改写为**非马尔可夫**形式，使反向过程可以**确定性**（{::nomarkdown}$\sigma_t = 0${:/nomarkdown}）：
...
- {::nomarkdown}$\sigma_t = 0${:/nomarkdown}：确定性映射，**DDIM**，可用远少于训练步数的步数采样（如 20–50 步）；
- {::nomarkdown}$\sigma_t = \sqrt{\frac{1-\bar\alpha_{t-1}}{1-\bar\alpha_t}}\sqrt{1 - \frac{\bar\alpha_t}{\bar\alpha_{t-1}}}${:/nomarkdown}：退化为 DDPM 的随机采样。

DDIM 的代价是通常略降低多样性，但显著加速推理，并支持**潜空间插值**（在 {::nomarkdown}$\mathbf{x}_T${:/nomarkdown} 上插值得到平滑过渡）。

---

## 7. Classifier-Free Guidance

CFG（Ho & Salimans, 2022）在**训练时**以概率 {::nomarkdown}$p_{\text{uncond}}${:/nomarkdown} 丢弃条件（如文本），使同一个网络同时建模条件与无条件分布。**推理时**：
...
其中 {::nomarkdown}$w \ge 1${:/nomarkdown} 为引导强度（{::nomarkdown}$w${:/nomarkdown} 越大结果越贴合条件、多样性越低）。直观上，网络在有条件与无条件两个方向上做外推，从而放大条件信息的影响。
...
- 先用预训练 VAE 将图像编码为潜变量 {::nomarkdown}$\mathbf{z} = \mathcal{E}(\mathbf{x})${:/nomarkdown}；
- 在潜空间运行 DDPM/DDIM；
- 用 VAE 解码器 {::nomarkdown}$\mathcal{D}${:/nomarkdown} 重建图像。
...
定义概率路径：从数据 {::nomarkdown}$\mathbf{x}_0${:/nomarkdown} 到噪声 {::nomarkdown}$\boldsymbol{\epsilon}${:/nomarkdown} 的线性插值：

$$\mathbf{x}_t = (1 - t)\,\mathbf{x}_0 + t\,\boldsymbol{\epsilon}, \qquad t \in [0, 1]$$

对 {::nomarkdown}$t${:/nomarkdown} 求导，目标速度场（velocity）为常数：

$$\dot{\mathbf{x}}_t = \boldsymbol{\epsilon} - \mathbf{x}_0$$

### 9.2 训练目标

用神经网络 {::nomarkdown}$v_\theta${:/nomarkdown} 回归速度场：
...
学得 {::nomarkdown}$v_\theta${:/nomarkdown} 后，用 ODE 求解器（如 Euler 或更高阶）从 {::nomarkdown}$\mathbf{x}_0 = \boldsymbol{\epsilon}${:/nomarkdown} 积分到 {::nomarkdown}$\mathbf{x}_1${:/nomarkdown}：
