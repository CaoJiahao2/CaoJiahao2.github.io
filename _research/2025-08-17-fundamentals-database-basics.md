---
title: "数据库基础"
description: "从关系模型与 SQL 到索引与事务，再到向量数据库与 RAG —— 数据管理的核心概念。"
date: 2025-08-17
...
没有索引时查询需全表扫描 {::nomarkdown}$O(n)${:/nomarkdown}；索引让查找接近 {::nomarkdown}$O(\log n)${:/nomarkdown}。
...
| 哈希索引 | {::nomarkdown}$O(1)${:/nomarkdown} 等值查找 | 主键/等值查询 |
| 位图索引 | 压缩、高效统计 | 低基数列 |
| 倒排索引 | 词 → 文档列表 | 全文检索 |
